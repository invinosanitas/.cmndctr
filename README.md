...
1. Copy a provider folder into your tool's config directory (for claude-code, use `settings.json` on Windows or `settings.unix.json` on macOS/Linux and name it `settings.json`)
2. Apply model tier mappings for that provider (see `model-tiers.json` and `scripts/apply-model-tiers.py`, add `--clean` to reset generated copies)
3. Restart the tool
